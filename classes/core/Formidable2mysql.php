...
class Formidable2mysql extends Formidable2RdbBase {
	...
	public function create_table( $table_name, $columns ) {
		if ( ! empty( $table_name ) ) {
			...
			if ( $error[0] == 0 ) {
				if ( ! $this->is_test() ) {
					do_action( "formidable2rdb_add_table", strtolower( $table_name ), $columns );
				}
			} else {
	...
	public function drop_table( $table_name ) {
		if ( ! empty( $table_name ) ) {
			...
			if ( $error[0] == 0 ) {
				if ( ! $this->is_test() ) {
					do_action( "formidable2rdb_drop_table", strtolower( $table_name ) );
				}
			} else {
	...
	public function get_columns( $table_name ) {
		if ( ! empty( $table_name ) ) {
			$tran = $this->execute( "DESCRIBE `" . $this->escape( strtolower( $table_name ) ) . "`" );
	...
	public function alter_table( $table_name, $args ) {
		if ( ! empty( $table_name ) ) {
			if ( ! empty( $args["add"] ) ) {
				$trans = $this->add_column( strtolower( $table_name ), $args["add"] );
				$error = $trans->errorInfo();
			}
			if ( ! empty( $args["change"] ) ) {
				$trans = $this->change_column( strtolower( $table_name ), $args["change"] );
			}
			if ( ! empty( $args["drop"] ) ) {
				$trans = $this->drop_columns( strtolower( $table_name ), $args["drop"] );
				$error = $trans->errorInfo();
			}
		} else {
			throw new Exception( "The table_name param is empty." );
	...
	public function exist_table( $table_name ) {
		if ( ! empty( $table_name ) ) {
			try {
				$result = $this->execute( "SELECT 1 FROM `" . $this->escape( strtolower( $table_name ) ) . "` LIMIT 1" );
				$result->closeCursor();
			} catch ( Exception $e ) {
				return false;
			}
			...
			try {
				if ( ! empty( $table_name ) ) {
					if ( ! empty( $column_name ) ) {
						$result = $this->execute( 'SHOW COLUMNS FROM `' . $this->escape( strtolower( $table_name ) ) . '` WHERE FIELDS=`' . $this->escape( $column_name ) . '`' );
						$result->closeCursor();
					} else {
	...
	public function insert( $table_name, $data ) {
		if ( ! empty( $table_name ) ) {
			...
			$error = $result->errorInfo();
			if ( $error[0] != 0 ) {
				throw new Formidable2RdbException( $error[2], array( Formidable2RdbManager::t( "Error inserting data to the table." ) . " " . strtolower( $table_name ) ) );
			}
	...
	public function update( $table_name, $data, $entry_id ) {
		if ( ! empty( $table_name ) ) {
			...
			$error = $result->errorInfo();
			if ( $error[0] != 0 ) {
				throw new Formidable2RdbException( $error[2], array( Formidable2RdbManager::t( "Error updating data into the table." ) . " " . strtolower( $table_name ) ) );
			}
	...
	public function exit_entry( $table_name, $entry_id ) {
		if ( ! empty( $table_name ) ) {
			$result = $this->execute( "SELECT COUNT(*) FROM `" . strtolower( $table_name ) . "` WHERE entry_id = " . $entry_id );
			$result->closeCursor();

			$error = $result->errorInfo();
			if ( $error[0] != 0 ) {
				throw new Formidable2RdbException( $error[2], array( Formidable2RdbManager::t( "Error checking entry into the table." ) . " " . strtolower( $table_name ) ) );
			}
	...
	public function delete( $table_name, $entry_id ) {
		if ( ! empty( $table_name ) ) {
			...
			$error = $result->errorInfo();
			if ( $error[0] != 0 ) {
				throw new Formidable2RdbException( $error[2], array( Formidable2RdbManager::t( "Error deleting data into the table." ) . " " . strtolower( $table_name ) ) );
			}
